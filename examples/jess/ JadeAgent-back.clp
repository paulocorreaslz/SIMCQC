...
;(deftemplate Gasolina (declare (from-class Gasolina)))
...
(defrule proposal
 ...
 ?m <- (ACLMessage (communicative-act CFP) (sender ?s) (content ?c) (receiver ?r))
  (test (eq ?c "Tem proposta para livros"))  =>
  (bind ?c "Tenho sim")
; (send (assert (ACLMessage (communicative-act proposal) (receiver ?s) (content ?c) )))
